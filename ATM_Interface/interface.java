import java.time.LocalDate;             //Importing Packages2121
...
class Main {
	...
	public static void main(String[] args)   //Main Method. The execution starts from this method
	{
		...
		String accname = "Janhavi.A.Patil";    //Declaring the account details
		String branchname = "Jalgaon";
		String branchaddress = "C-67843,DHULE ROAD,Ramanand Colony,Jalgaon";
		String branchIFSC = "OBC785132";
		
		int accnumber = 82491635;
		int password = 8529;
		int balance = 0;
		int minbal = 500;
		boolean flag = true;
		...
		if(currattempt<4)     // Executing the main task if users enters correct password within limited attempts
		{
			while(flag==true)
			{
				System.out.println("************");
						...
						while(true)
						{
							...
							if((withdraw % 10) ==0)
							{
								if(withdraw <= balance)
								{
									...
									{
										System.out.println("Minimum balance has to Rs 500");
									}
									else
									{
								...
								System.out.println("Enter the withdraw amount in multiples of 500");
						}
